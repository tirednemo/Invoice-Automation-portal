    ...
    <form method="POST" action="{{ route('invoices.store') }}" onsubmit="return validateForm()">
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2">
                    ...
                    <input id="merchant_name" type="text" name="merchant_name" value="{{ $pdfData['customer_info']['name'] }}" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>
            </div>
        </div>
                ...
                <div id="item-details-container">
                    ...
                    <div class="grid grid-cols-5 gap-3 item-details-row">
                        <div class="col-span-1">
                            ...
                            <input required type="text" name="item_details[{{$index}}][name]" value="{{ $item['name'] }}" class="mb-2 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                        <div class="col-span-1">
                            <label class="block text-gray-700 text-sm font-bold mb-1">Unit Price</label>
                            <input required type="number" name="item_details[{{$index}}][unit_price]" value="{{ $item['unit_price'] }}" class=" mb-2 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                        <div class="col-span-1">
                            <label class="block text-gray-700 text-sm font-bold mb-1">Quantity</label>
                            <input required type="number" name="item_details[{{$index}}][quantity]" value="{{ $item['quantity'] }}" class=" mb-2 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                        <div class="col-span-1">
                            <label class="block text-gray-700 text-sm font-bold mb-1">Amount</label>
                            <input required type="number" name="item_details[{{$index}}][amount]" value="{{ $item['amount'] }}" class=" mb-2 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                        <div class="col-span-1">
        ...
        <div class="mb-4">
            <label for="status" class="inline-flex items-center">
                <input type="checkbox" id="status" name="status" class="form-checkbox h-4 w-4 text-blue-500 focus:ring-blue-500 focus:border-blue-500 border-gray-300 rounded">
